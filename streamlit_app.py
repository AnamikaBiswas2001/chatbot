...
def save_estimation_to_history(project_title, total_cost, df_roles, source):
    try:
        conn = snowflake.connector.connect(**st.secrets["snowflake"])
        cursor = conn.cursor()
        ...
        query = f"""
            INSERT INTO rfp_estimation_history (project_title, total_cost, roles, timestamp, source)
            VALUES (%s, %s, %s, %s, %s)
        """
        cursor.execute(query, (project_title, float(total_cost), json_roles, current_time, source))
        conn.commit()
        cursor.close()
        conn.close()
    except Exception as e:
...
# --- Chat Tab ---
with tabs[0]:
    user_input = st.text_input("Enter project-related question or task description:", key="user_input")
    if user_input:
        ...
        if keyword:
            ...
            if not df_roles.empty:
                ...
                save_estimation_to_history("Chat Query", total, df_roles, source="chat")
                st.session_state.chat_history.append({"user": user_input, "assistant": f"Estimated cost: ${total:,.2f}"})
            else:
...
with tabs[1]:
    ...
    if doc_file:
        ...
        df_roles = structured_df if not structured_df.empty else fetch_roles_for_keyword(extract_semantic_keyword(text, keywords))
        ...
        if not df_roles.empty:
            ...
            for k, v in project_info.items():
                st.markdown(f"**{k}:** {v}")
            total = display_estimate(df_roles)
            save_estimation_to_history(project_info.get("Project Title", "Untitled RFP"), total, df_roles, source="document")

            st.markdown("### 📝 Responses to Proposal Requirements")
            for req in extract_proposal_requirements(text):
                st.markdown(f"**• {req}**")
...
with tabs[2]:
    try:
        conn = snowflake.connector.connect(**st.secrets["snowflake"])
        history_df = pd.read_sql("SELECT project_title, total_cost, roles, source, timestamp FROM rfp_estimation_history ORDER BY timestamp DESC", conn)
        if not history_df.empty:
            st.markdown("### 📚 Past Estimations")
            for _, row in history_df.iterrows():
                st.markdown(f"**Project:** {row['PROJECT_TITLE']} | **Total:** ${row['TOTAL_COST']:,.2f} | **Source:** {row['SOURCE']} | **Timestamp:** {row['TIMESTAMP']}")
